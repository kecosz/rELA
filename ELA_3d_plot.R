...
df = read.csv(file="onw_table.csv")
dim(df)[[1]]
...
size_of_numbers = 30
red_color_no_index = 22# OCZ
blue_color_no_index = 99# OSNnumber_of_samples #146
...
frame6 <- plot_ly(data=df, x = ~rel.MDS1, y= ~rel.MDS2, z= ~Energy) %>% 
  add_trace(data=df, x = ~rel.MDS1, y= ~rel.MDS2, z= ~Energy,
            ...
            name="Samples",
            opacity = 1)  %>% 
  add_trace(data=df[df$time==red_color_no_index,], x = ~rel.MDS1, y= ~rel.MDS2, z= ~Energy,
            ...
            name="SS1: OCZ",
            opacity = 1) %>%   
  add_trace(data=df[df$time==blue_color_no_index,], x = ~rel.MDS1, y= ~rel.MDS2, z= ~Energy,
            #text =c(1:number_of_replicates), mode = "text",
            text =rep(c("v"),each=number_of_replicates), mode = "text",
